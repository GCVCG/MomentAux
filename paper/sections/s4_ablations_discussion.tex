...
\midrule
loss: MSE $\rightarrow$ cosine & $+3.31 \rightarrow$ & $+2.81 \rightarrow +0.46$ & magnitude scale matters \\
\bottomrule
\end{tabular}
\end{table}
...
regularisation''. A FitNets teacher trained on the same data at twice the
...
only realise it. Head-norm projection, derived from a traced scale
degeneracy (the aux loss is invariant under feature-shrink/head-inflate,
...
\subsection{Stabilisation as a second, distinct effect}
...
$4.3$ vs.\ $0.2$ with the prior). We report stabilisation as a real,
...
ViT/Swin from scratch, any scale & prior & $1.02\times$ & $+3.2$ at 1.28M imgs; $+13/{+}26$ at 224\,px; stabilises Swin \\
...
readout channel whose behaviour is set by how well the task is already
...
Our stack/substitute/tax taxonomy is, we suggest, the general shape of
...
plausibly an interaction between late-stage optimisation and
...
are budget-bound by construction. One prior family (Gabor/moment energy)
...
Food-101/PathMNIST cells to locate where good features fail to become
...
scale}: the model-scale trend ($+13 \to +26$ from ViT-S to ViT-B at fixed
data) invites the obvious next point on the curve, and the
stabilisation-plus-structure account suggests trying the prior as a
...
magnitude in data. The measurements organise under one empirical law
