...
declared compute. The prior, which we call \emph{MomentAux}, could hardly cost less: a fixed bank of oriented-energy targets, injected only during
...
$5\times$ narrows the flip to the higher-data band, where the prior still
wins on both populations, while the comparator takes the scarcest cells
(Section~\ref{sec:budget}); the ordering therefore tracks the data
fraction, and the comparison is a statement about budget and regime, not a
ranking of methods.
...
\begin{enumerate}
  ...
  \item \textbf{The law} (Section~\ref{sec:law}): $\Delta = G +
    ...
    ImageNet scale, with its boundary cases reported as findings rather
    than excluded --- including the one patterned failure, where features
    improve at sufficiency while accuracy does not follow
    (Section~\ref{sec:exceptions}). The same account resolves an apparent
    puzzle at ImageNet scale: three backbones on identical data give three
    different envelope shapes, and the difference is only where each sits
    on its own readout curve (Section~\ref{sec:scaleenvelope}).
...
linear-evaluation gap on frozen features: inexpensive enough for \computeCells{} cells,
...
manufacture illusory progress, and reproducibility in machine learning remains limited. We adopt
