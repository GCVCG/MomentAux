...
question needs, and what this work adds. P1: one frozen recipe. P2:
cost-normalized, comparators reported against a declared training-compute
multiple, not each method's own chosen budget. P3: feature-side, the effect
measured on frozen representations, not only end-to-end. P4: combinations,
the study measures what happens when two sources are applied together. P5:
a predictive model of the outcome. \cmark{} = present; \pmark{} = partial
or incidental; \xmark{} = absent.}
\label{tab:gap}
