...
measurable, on frozen features, from each source \emph{alone}, which is what
separates the three outcomes above and why the same free prior can be the most
valuable intervention we measure in one setting and the most damaging in
another. Turning that separation into a rule that \emph{predicts} an untrained
combination is a further step, and one we take and report as unsuccessful
(Sec.~\ref{sec:procedure}).
...
and SimCLR each render the other nearly worthless, and their combination sits
at the better single, above it by at most a point where at all. Different-currency fusions can stack, the prior's gain
...
\begin{enumerate}
  ...
  \item \textbf{A procedure, stated and then falsified}
    (Sec.~\ref{sec:procedure}): because each source's currency is
    measurable from that source \emph{alone}, we state the taxonomy as an
    algorithm over quantities available before the combination is trained,
    and test it on thirteen pairs whose combination did not yet exist, with
    every input estimated on validation data and the calls recorded first.
    It calls one of nine correctly, so we withdraw the predictive claim,
    report which predicate fails and why, and distil the measured grid into a
    regime-indexed guide with costs attached (Sec.~\ref{sec:guide}).
\end{enumerate}

\looseness=-1 In one line, what the grid says: each source's currency is
measurable on its own, but comparing those measurements does not tell you
which of the three outcomes their fusion produces, and
Sec.~\ref{sec:procedure} is the held-out test that showed it; pouring a
shaping prior into a strong initialization costs $-15$ to $-17$ points at full
auxiliary strength, and that cost tracks the strength rather than the fusion,
falling away below it (Sec.~\ref{sec:taxsub}); modern augmentation does not
replace structure but amplifies it (Sec.~\ref{sec:stack}); attention's feature
deficit grows with model scale rather than shrinking (Sec.~\ref{sec:scale});
and a comparator's budget is part of the claim, since two of ours did not
survive raising it (Sec.~\ref{sec:budget}).
...
question needs, and what this work adds. \cmark{} = present; \pmark{} =
partial or incidental; \xmark{} = absent.}
\label{tab:gap}
\centering\scriptsize
\setlength{\tabcolsep}{2.6pt}
\zebra{2}
\begin{tabular}{P{0.58\columnwidth}ccccc}
\toprule
Line of work & Rec. & Cst. & Fea. & Cmb. & Prd. \\
\midrule
...
\bottomrule
\rowcolor{white}\multicolumn{6}{@{}p{0.97\columnwidth}@{}}{\footnotesize Rec.: one frozen recipe. Cst.: cost-normalized, comparators reported against a declared training-compute multiple, not each method's own chosen budget. Fea.: feature-side, the effect measured on frozen representations, not only end-to-end. Cmb.: combinations, what happens when two sources are applied together. Prd.: a predictive model of the outcome.} \\
\end{tabular}
\end{table}
