...
actively destroy value? These three outcomes, we will call them
\emph{stack}, \emph{substitute}, and \emph{tax}, are usually discovered
anecdotally, one paper and one setting at a time, under training recipes
that differ in a dozen uncontrolled ways.
...
run, and a numerically pinned bank of Gabor and moment
...
resolutions from $32$ to $224$\,px, and 2{,}894 configurations trained
over 8{,}827 runs. The
...
The law converts the grid into an explanation, and it is where the
currencies stop being a metaphor and become the term $G$. Each
intervention's is legible in what it does to frozen features: heavy augmentation supplies
...
\begin{enumerate}
  ...
  \item \textbf{The grid} (Section~\ref{sec:grid}): $2{,}894$ configurations,
  $8{,}827$ runs,
...
\paragraph{Takeaways.} For a reader who reads no further:
\begin{enumerate}
  \item \textbf{Measure the currency, not the method.} A linear
    evaluation of each source \emph{alone} predicts whether combining
    them will stack, do nothing, or hurt. Sources whose feature gains are
    close are substitutes, however different their machinery
    (Section~\ref{sec:fusion}).
  \item \textbf{Never fuse a shaping prior into a strong
    initialization.} The tax is feature-side and proportional to what the
    initialization already supplied: $-17$ points where ImageNet transfer
    is worth the most, and indistinguishable from zero on
    histopathology, where it is worth least (Section~\ref{sec:taxsub}).
  \item \textbf{Modern augmentation does not replace structure, it
    amplifies it.} The same recipe that makes $2\times$-compute
    contrastive pre-training nearly redundant makes the free prior worth
    $1.4$--$2.4\times$ more, because the two supply different currencies
    (Section~\ref{sec:stack}).
  \item \textbf{Attention's deficit grows with model scale.} $+13.0$ for
    ViT-S/16 and $+26.0$ for ViT-B/16 at $224$\,px, and still $+3.2$ after
    1.28M images, where our ResNet-18 is neutral ($+0.04$). Scaling the
    model does not remove the need for the prior; it increases it
    (Section~\ref{sec:scale}).
  \item \textbf{Declare the comparator's budget, or the conclusion is
    about the budget.} SimSiam appears to learn nothing at its published
    budget and gains $+4.3$ at four times that; two of our own accuracy
    claims did not survive the same test and are restated as cost claims
...
guidance exists for large-scale use. Much
...
photo-like data, with known caveats under
...
\citep{touvron2021deit}, convolutional biases and hybrid designs, and
auxiliary self-supervision. That the deficit
...
a scattering-based transformer performs
...
linear-evaluation gap on frozen features: cheap enough for 2{,}894 cells,
...
manufacture illusory progress, and reproducibility in machine learning
remains fragile. We adopt
