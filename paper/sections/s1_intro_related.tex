...
in the field \citep{khaleghi2013multisensor,shaik2024survey} states the triple as
...
for a fixed fusion problem \citep{zhang2021benchmarking}; ours compares
\emph{sources} under a fixed algorithm, holding the recipe frozen so that
what varies is the knowledge injected rather than the method injecting it.

One difference is substantive rather than terminological, and it sharpens
...
\citep{wang2019negative,zhang2023negative}; and on the multimodal side it
...
linear-evaluation gap on frozen features: cheap enough for 2{,}800 cells,
ordinal rather than an information quantity, and blind to anything not
...
Two further connections position the predictor rather than the taxonomy.
Predicting a downstream outcome from a cheap measurement, without training
the combined system, is the programme of transferability estimation
\citep{nguyen2020leep,you2021logme}, which scores a single pre-trained
...
with fine-tuning performance in general. Our defence is narrow and
empirical rather than a rebuttal. We do not use the linear evaluation to
...
sources'' under which this study is a fusion study at all. Ours is
...
their own informed pre-training \citep{vonrueden2022informedpre}, which
