...
| `results/best_epoch/` | the ARCHIVED best-epoch corpus (`all_results_best.csv`, `results_by_portion_best.csv`, `law_audit_best.md`): the same tables with every `G` from `best.pt`, superseded on 2026-09-09 by the matched-epoch corpus above (every `G` from the final-epoch checkpoint, the network `Delta` describes); plus the best-epoch records of the imprint, currency, attention-locality and CAM diagnostics (`*_best.json`), superseded on 2026-09-10 when those scripts were re-run on the same final-epoch checkpoints (`--ckpt last.pt`). Kept so both swaps are checkable, not a source of any released number. |
...
```bash
python analysis/audit_law_paired.py      # 955 in scope, 485 resolvable, 408 correct (84.1%)
```
...
- `linear_probe_last.json` — per cell. The full-train-set linear probe of the
  final-epoch checkpoint, the network whose accuracy `final.json` records; the
  measurement behind every released `G` (one evaluation protocol: `Delta` and
  `G` on the same weights).
- `linear_probe.json` — per cell. The same probe on `best.pt`, the ARCHIVED
  best-epoch corpus (`results/best_epoch/`). Kept for the record; not the
  source of any released table.
