...
machine-checked from all seeds by `analysis/audit_law_paired.py`, the
canonical audit. Every figure shows **human class
...
teachers, random maps) on CIFAR-100; off the selection set the ordering
compresses and the surviving claim is the margin over random fixed maps.
...
**CIFAR-100@5%** — the champion's peak cell (Δe2e +5.15, G +6.26 at ten seeds). The aux
...
```bash
python analysis/audit_law_paired.py   # writes results/law_audit.md (canonical)
```

| figure | cell | Δe2e | G (probe gap) | readout |
|---|---|---|---|---|
| §2, §3, §4 | tin@1% | +1.49 ±0.09 | +4.19 ±0.15 | −2.69 |
| §2, §3, §4 | C100@5% | +5.15 ±0.22 | +6.26 | −1.11 (ten seeds; the figures use seed 0) |
