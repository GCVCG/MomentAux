...
demonstrates about the method. Regenerate any of them with:

```bash
python analysis/visualize_features.py --pair <baseline_cell> <aux_cell> --out docs/viz
```

All figures use **seed 0** of each cell and the **test split**; they are
diagnostics, never headline evidence — the quantitative claims they illustrate
are machine-checked from all seeds by `analysis/audit_law.py`
