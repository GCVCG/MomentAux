...
Supplementary material for *When does fusing hand-crafted knowledge with learned representations pay? A cost-normalized benchmark of stacking, substitution and interference* (AlMughrabi, Clop, Busam, Marques, Radeva).
...
Built from repository commit `cdab285` (2026-09-09) by `python scripts/make_release_assets.py --out dist/`. Tarballs are byte-reproducible (sorted members, fixed mtimes), so the checksums below identify this exact build.
...
| `logs.tar.gz` | 1.7 MB | 388 | campaign logs: what was submitted when, and what failed |
| `result-tables.tar.gz` | 1.6 MB | 183 | aggregated tables, the law audit, and the per-analysis JSON records |
| `run-records.tar.gz` | 1.9 MB | 26,267 | every run's authoritative record: config as executed, accuracy, FLOPs, environment, and the probes behind G |
| `training-curves.tar.gz` | 35.6 MB | 9,965 | per-epoch train/test accuracy, loss components, the lambda schedule |
...
5eb678c38f834fa91d05e09cbfe8d0930697dd44243c541b14e1fbde46a38dd7  logs.tar.gz
567765c80c01a924aaeeb15b27f4ad4abb43650131502178d0b2f69ad03e6fa4  result-tables.tar.gz
df99cff511d191e09292e2fe58f6eedf7a1e5c07dbc39bd280ac40c68314a511  run-records.tar.gz
2027becfad4b052c4ccc955505722f772f570dc479361a5525f0a77602bf7537  training-curves.tar.gz
```
