...
#
# WHY THIS EXISTS. The det grid was hand-fed three times in one day, and each
...
# next pass. Duplicates are caught by train_det.py's final.json guard and its
...
if ! python3 "$MS/det_missing.py" > "$WL.new" 2>"$MS/logs/det_keeper.err"; then
    echo "$(stamp) STATE ERROR: generator FAILED, touching nothing"; exit 1
fi
M=$(wc -l < "$WL.new")

if [ "$M" -eq 0 ]; then
    rm -f "$WL.new"; touch "$MS/DET_COMPLETE"
    echo "$(stamp) STATE det grid COMPLETE (0 missing)"; exit 0
fi
