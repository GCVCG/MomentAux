...
DR=${DATA_ROOT:-data}
PCT=${PCT:-5}
METHOD=${METHOD:?set METHOD=simclr or simsiam}
LOG=logs/sslbudget_par_${METHOD}_${PCT}.log
: > "$LOG"
one() {
  local seed=$1
  local out="runs/${METHOD}_pre800_c100_${PCT}pct/seed${seed}"
  if [ ! -f "$out/pretrain.pt" ]; then
    mkdir -p "$out"
    $PY scripts/${METHOD}_pretrain.py --data-root "$DR" \
        --config "configs/sslbudget/diagsslbudget_${METHOD}800_c100_${PCT}pct.yaml" \
        --epochs 800 --seed "$seed" --out "$out/pretrain.pt" \
        >> "logs/${METHOD}_pre800_c100_${PCT}pct_s${seed}.log" 2>&1
    echo "$(date -Is) pretrain seed$seed rc=$?" >> "$LOG"
  fi
  $PY train.py --config "configs/sslbudget/diagsslbudget_${METHOD}800_c100_${PCT}pct.yaml" \
      --seed "$seed" --data-root "$DR" \
      >> "logs/sslbudget_${METHOD}_${PCT}pct_s${seed}.log" 2>&1
